...
`cidr`, `inet`, `macaddr` and `citext` column types (PostgreSQL 9.3 and above).
The `citext` type requires the `citext` extension to be enabled on the database;
the migration does not create the extension automatically — run
`CREATE EXTENSION IF NOT EXISTS citext` first (for example from an earlier
migration via `$this->execute(...)`).
